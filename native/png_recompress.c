...
int recompress_png_idat(
  ...
  memcpy(out + w, ihdr, 13); w += 13;
  {
    ...
    _write_u32_be(out + w, crc); w += 4;
  }

  // IDAT
  _write_u32_be(out + w, (uint32_t)comp_len); w += 4;
  out[w++] = 'I'; out[w++] = 'D'; out[w++] = 'A'; out[w++] = 'T';
  memcpy(out + w, compressed, (size_t)comp_len); w += (size_t)comp_len;
  {
...
int recompress_png_batch(
  ...
  if (requested > count) requested = count;
  ...
  if (work.failed) {
    for (int32_t i = 0; i < count; i++) {
      if (item_outputs[i]) free_native_buffer(item_outputs[i]);
    }
    free(item_outputs);
    free(item_sizes);
    free(out_offs);
  ...
  for (int32_t i = 0; i < count; i++) {
    if (!item_outputs[i] || item_sizes[i] <= 0) {
      for (int32_t j = 0; j < count; j++) {
        if (item_outputs[j]) free_native_buffer(item_outputs[j]);
      }
      free(item_outputs);
      free(item_sizes);
      free(out_offs);
