...
class ConversionAnalytics {
  ...
  Duration get totalStageTime => stages.values.fold(
      Duration.zero, (acc, d) => acc + d);

  Map<String, double> get stagePercentages {
    final totalMs = totalStageTime.inMilliseconds.toDouble();
    if (totalMs <= 0) return {};
    return stages.map((k, v) => MapEntry(
        k, (v.inMilliseconds.toDouble() / totalMs) * 100.0));
  }

  static ConversionAnalytics fromWorkerMap(Map<String, dynamic> data) {
    final stageNs =
        Map<String, dynamic>.from(data['stagesNs'] as Map? ?? {});
    final nativeStageNs =
        Map<String, dynamic>.from(data['nativeStagesNs'] as Map? ?? {});
    ...
    final nativeStages =
        nativeStageNs.map((k, v) => MapEntry(k, _durFromNs(v)));
    ...
    for (int i = 0; i < workersRaw.length; i++) {
      ...
      workerTimings.add(WorkerTiming(
  ...
  int calculateOptimalWorkerCount() {
    if (workerTimings.isEmpty) return cpuCores;
    
    final activeWorkers =
        workerTimings.where((w) => w.layers > 0).toList();
    ...
    return efficientWorkers.clamp(
      ...
      (cpuCores * 2.0).ceil(), // At most 2x cores
    );
  }
}
...
List<DiagnosisItem> _analyze(ConversionAnalytics a) {
  ...
  if (workers > cores) {
    ...
    if (score >= 10) {
      items.add(DiagnosisItem(
        ...
        detail: 'Workers ($workers) exceed logical cores ($cores). '
  ...
  if (a.workerTimings.isNotEmpty) {
    final activeWorkers =
        a.workerTimings.where((w) => w.layers > 0).toList();
    final totals =
        activeWorkers.map((w) => w.total.inMilliseconds.toDouble());
    ...
    if (activeWorkers.length >= 3) {
      ...
      final avgLayers = layerCounts.reduce((a, b) => a + b) / layerCounts.length;
      ...
      if (deviationPct > 5) {
        ...
        if (slowWorkers.length >= 3) {
          ...
          items.add(DiagnosisItem(
            ...
            detail: 'Layer distribution: ${minLayers}-${maxLayers} (${deviationPct.toStringAsFixed(1)}% spread). '
  ...
  if (readPct > 0.35) {
    items.add(DiagnosisItem(
      ...
      detail: 'Reading layers dominates runtime. Consider preload, faster disk, '
  ...
  if (writePct > 0.30) {
    items.add(DiagnosisItem(
      ...
      detail: 'Writing the NanoDLP ZIP dominates runtime. Fast storage or '
  ...
  if (recompressPct > 0.25) {
    items.add(DiagnosisItem(
      ...
      detail: 'Recompress pass is expensive. Try recompress mode “adaptive/off”.',
      score: (recompressPct * 100).clamp(0, 100).toDouble(),
    ));
  }

  final nativeTotalMs = a.nativeStages.values
      .fold(0.0, (acc, d) => acc + d.inMilliseconds.toDouble());
  if (nativeTotalMs > 0) {
    ...
    if (compressPct > 0.45) {
      items.add(DiagnosisItem(
        ...
        detail: 'Zlib compression dominates native time. Reduce PNG level '
            'or enable fast mode.',
  ...
  if (a.gpuAttempts > 0) {
    final failPct =
        (a.gpuAttempts - a.gpuSuccesses) / a.gpuAttempts.toDouble();
    if (failPct > 0.15) {
      items.add(DiagnosisItem(
        title: 'GPU fallbacks',
        detail: 'GPU attempts frequently fall back to CPU. Verify drivers or '
            'use CPU native mode.',
